...
def get_price_for_interval(df, t, uuid, duration):
    ...
    if t < int(df.first_valid_index().timestamp()):
        price_duration_list.append((int(df.first_valid_index().timestamp()), df.iloc[0][uuid],
                                    duration + t - int(df.first_valid_index().timestamp())))
        return price_duration_list
            ...
            if t + duration <= int(index.timestamp() + 3600):
                ...
                if index == df.last_valid_index():
                    ...
                    if index.timestamp() + 3600 == next_index.timestamp():
                        ...
                        price_duration_list.append(
                            (int(next_index.timestamp()), next_row[uuid], int(t + duration - next_index.timestamp())))
                        return price_duration_list
                    else:
                        if t > int(index.timestamp()) and t < int(index.timestamp() + 3600):
                            price_duration_list.append((t, row[uuid], int(index.timestamp() + 3600) - t))
...
def get_price(request, pymortar_client):
    ...
    print("received request:", request.utility, request.tariff, request.price_type, request.start, request.end,
          request.window)
    ...
    if request.start / 1e9 % 3600 == 0:
        ...
        if duration == 3600:
            for index, row in df.iterrows():
                prices.append(price_pb2.PricePoint(time=int(index.timestamp() * 1e9), price=row[uuid], unit=unit,
            ...
            for index, row in df.iterrows():
                for i in range(int(index.timestamp()), int(index.timestamp()) + 3600, duration):
                    if i + duration <= request.end / 1e9:
                        prices.append(
                            price_pb2.PricePoint(time=int(i * 1e9), price=row[uuid], unit=unit, window=request.window))
        ...
        if duration == 3600:
            ...
            for index, row in df.iterrows():
                if i > index.timestamp():
                    prices.append(price_pb2.PricePoint(time=int(i * 1e9), price=row[uuid], unit=unit,
                                                       window=get_window_in_string(index.timestamp() + 3600 - i)))
                ...
                if i > index.timestamp():
                    ...
                    if part_duration % duration != 0:
                        ...
                        if i + new_duration - part_duration <= request.end / 1e9:
                            prices.append(price_pb2.PricePoint(time=int(i * 1e9), price=row[uuid], unit=unit,
                                                               window=get_window_in_string(
                                                                   new_duration - part_duration)))
        ...
        for i in range(int(request.start / 1e9), int(request.end / 1e9), duration):
            ...
            for t, p, d in pd_list:
                if t + d <= request.end / 1e9 and t + duration <= request.end / 1e9:
                    prices.append(
                        price_pb2.PricePoint(time=int(t * 1e9), price=p, unit=unit, window=get_window_in_string(d)))

...
class LTIOcc(ParentThermalModel):
    ...
    def __init__(self, interval_thermal, thermal_precision=0.05, learning_rate=0.00001):
        ...
        super(LTIOcc, self).__init__(thermal_precision, interval_thermal)
    ...
    def _features(self, X):
        ...
        Tin, action, previous_action, action_duration, Tout, occ, dt, zone_temperatures = X[0], X[1], X[2], X[3], X[4], X[5], X[6], X[7:]
        features = []


        if not LTIOcc.TWO_STAGES:
            NUM_ACTIONS = 2 # heating and cooling
            features += [Tin for _ in range(NUM_ACTIONS*LTIOcc.NUM_START + NUM_ACTIONS*LTIOcc.NUM_END)]
    ...
    def _filter_actions(self, X):
        ...
        action, previous_action, action_duration, occ, dt, zone_temperatures = X[1], X[2], X[3], X[5], X[6], X[7:]

        action_filter = []

        # starts (as in param_order)
        for iter_action, iter_num_start in itertools.product([utils.HEATING_ACTION, utils.COOLING_ACTION], list(range(LTIOcc.NUM_START))):
            ...
            if iter_num_start != LTIOcc.NUM_START - 1:
                action_filter += [(action == iter_action) & (((action_duration - dt) // self.interval_thermal) == iter_num_start)]
            else:
                action_filter += [(action == iter_action) & (((action_duration - dt) // self.interval_thermal) >= iter_num_start)]

        # ends (as in param_order)
        for iter_action, iter_num_end in itertools.product([utils.HEATING_ACTION, utils.COOLING_ACTION], list(range(LTIOcc.NUM_END))):
            ...
            # the duration until the end of the interval.
            if iter_num_end != LTIOcc.NUM_END - 1:
    ...
    def fit(self, X, y):
        ...
        filter_columns = ['t_in', 'action', 'previous_action', 'action_duration', 't_out', 'occ', 'dt'] + list(zone_col)
        ...
        if not LTIOcc.TWO_STAGES:
            actions_order = [act + "_" + "start" + "_" + str(num) for act, num in itertools.product(["heating", "cooling"], list(range(LTIOcc.NUM_START)))]
            actions_order += [act + "_" + "end" + "_" + str(num) for act, num in itertools.product(["heating", "cooling"], list(range(LTIOcc.NUM_END)))]

            self._params_coeff_order = actions_order + ['t_out', 'no_action', 'occ'] + list(zone_col)

            self._params_bias_order = actions_order + ['t_out', 'no_action', 'occ'] + list(zone_col)
        else:
            raise NotImplementedError("Two stage thermal model is not implemented.")

...
class TestTemperatureData(unittest.TestCase):   
    ...
    def get_response(self, building="ciee", zone="HVAC_Zone_Eastzone", window="1h", start=-1, end=-1):
        try:
            ...
            return xbos.get_indoor_temperature_historic(stub, building=building, zone=zone, start=start,end=end,window=window)
    ...
    @test_utils.all_buildings_and_zones(max_interval_days=1, log_csv="temp_tests.csv", buildings=buildings, zones=zones)
    ...
    @test_utils.random_buildings_and_zones(iterations=2, log_csv="temp_random_tests.csv", buildings=buildings, zones=zones)

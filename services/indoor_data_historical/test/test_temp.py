...
class TestTemperatureData(unittest.TestCase):   
    ...
    def get_response(self, building="ciee", zone="HVAC_Zone_Eastzone", window="1h", start=-1, end=-1):
        try:
            ...
            return self.stub.GetRawTemperatures(indoor_temperature_action_pb2.Request(building=building, zone=zone, start=start,end=end,window=window))
        except grpc.RpcError as e:
            print(e)
    
    def response_exists(self, response):
        self.assertIsNotNone(response)
        self.assertIsInstance(obj=response, cls=indoor_temperature_action_pb2.RawTemperatureReply)
    ...
    def valid_data_exists(self, response, window):
        ...
        for temp in response.temperatures:
            ...
            self.assertIsNotNone(temp.time)
            self.assertIsNotNone(temp.temperature)
            self.assertIsNotNone(temp.unit)
            self.assertIsInstance(obj=temp, cls=indoor_temperature_action_pb2.TemperaturePoint)
            self.assertIsInstance(obj=temp.time, cls=int)
            self.assertIsInstance(obj=temp.temperature, cls=float)
            self.assertIsInstance(obj=temp.unit, cls=str)
            last_time = self.window_is_accurate(last_time, temp.time, window)
...
if __name__ == '__main__':
    with grpc.insecure_channel('localhost:50060') as channel:
        ...
        for test_name in test_names:
            suite.addTest(TestTemperatureData(test_name, stub))

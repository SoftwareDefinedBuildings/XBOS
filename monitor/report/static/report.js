    ...
    $(document).ready(function() {
        ...
        $('.dailyplot').each(function(idx) {
            ...
            d3.json("/zonedata/"+key+"/"+zone, function(err, data) {
                ...
                var miny_a = d3.min(data_temp, function(d) { return d.value; });
                var maxy_a = d3.max(data_temp, function(d) { return d.value; });
                var miny_b = d3.min(data_temp_cool, function(d) { return d.value; });
                var maxy_b = d3.max(data_temp_cool, function(d) { return d.value; });
                var miny_c = d3.min(data_temp_heat, function(d) { return d.value; });
                var maxy_c = d3.max(data_temp_heat, function(d) { return d.value; });
        ...
        //$('#hvacdemandplot')
        /*
        d3.json("/hvacdemandplot", function(err, data) {
            var margin = {
                top: 50,
                right: 50,
                bottom: 50,
              ...
              var y_demandAxis = d3.svg.axis()
                  .scale(y_demand)
                  .orient("left");
              var y_hvacAxis = d3.svg.axis()
                  .scale(y_hvac)
                  .orient("right");
              var demand_line = d3.svg.line()
                  .x(function(d) { return x(d.date); })
                  .y(function(d) { return y_demand(d.value); });
              var hvac_line = d3.svg.line()
                  .x(function(d) { return x(d.date); })
                  .y(function(d) { return y_temp(d.value); });
              var svg = d3.select("#hvacdemandplot").append("svg")
                  .attr("width", width+margin.left+margin.right)
                  .attr("height", height+margin.top+margin.bottom)
                .append("g")
                  .attr("transform","translate("+margin.left+","+margin.top+")");

              data_hvac = data['hvac'].sort(function(a,b) { return b.date - a.date; });
              data_hvac.forEach(function(d) {
                  d.date = moment(new Date(d['date']*1)).add(7,'hours').toDate();
                  d.value = d['value'];
              });
              data_demand = data['demand'].sort(function(a,b) { return b.date - a.date; });
              data_demand.forEach(function(d) {
                  d.date = moment(new Date(d['date']*1)).add(7,'hours').toDate();
                  d.value = d['value'];
              });
        });
